...
输入一名角色后，系统会比较目标与本次猜测的六项信息：**属性、命途、稀有度、主派系、地区和实装版本**。根据每一格反馈继续缩小范围，在次数用尽前找出答案。
...
普通角色每日、练习和正式匹配固定 **6 次**猜测；其它内容模式使用各自 manifest 声明的固定次数。
...
| **练习**     | 每局抽取一个新目标，不影响每日题记录；同一内容模式只保留一局进行中的练习题           |
| **实时对战** | 支持 BO1 / BO3 / BO5 / BO7、随机匹配和私人房间；注册账号可参加排位，访客可加入好友房 |

每日题和练习都会先进入准备页；只有确认开始后才创建或恢复对局并计时。离开页面不会暂停服务器对局，重新打开时可以继续。

## 功能亮点

- 总站大厅平级展示每日题、无限随机和实时对战，并明确标记未开始、进行中和已完成状态；
- 支持刷新续局、历史回放、个人统计、Elo 排行榜，以及仅含判定格与网站二维码的无剧透结果图片分享；
