-- 普通角色无尽局冻结题池、规则和洗牌袋进度，保证续局与历史裁决稳定。
CREATE TABLE endless_runs (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id),
  mode_id TEXT NOT NULL DEFAULT 'playable' CHECK (mode_id = 'playable'),
  ...
  consumed_target_ids_json TEXT NOT NULL DEFAULT '[]'
    CHECK (json_valid(consumed_target_ids_json)),
...
CREATE UNIQUE INDEX endless_runs_active_user_mode_idx
  ON endless_runs(user_id, mode_id)
  WHERE status = 'active';
