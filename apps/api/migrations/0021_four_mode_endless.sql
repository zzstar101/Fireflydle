-- 扩展无尽局模式约束，同时重建猜测表以保持外键指向新表。
ALTER TABLE endless_guesses RENAME TO endless_guesses_playable;
ALTER TABLE endless_runs RENAME TO endless_runs_playable;

CREATE TABLE endless_runs (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id),
  mode_id TEXT NOT NULL DEFAULT 'playable'
    CHECK (mode_id IN ('playable', 'npc', 'currency-wars', 'aeon')),
...
INSERT INTO endless_runs SELECT * FROM endless_runs_playable;
...
INSERT INTO endless_guesses SELECT * FROM endless_guesses_playable;
DROP TABLE endless_guesses_playable;
DROP TABLE endless_runs_playable;
