...
const copy = {
  "zh-CN": {
    ...
    dataTitle: "数据与账号",
    data: "访客标识、游戏结果和隐藏匹配分用于提供游戏。注册或登录时，访客记录会安全合并到账号。登录名保持私密；展示名、公开 Elo 与符合资格的排行成绩会公开。邮箱可选，只用于找回密码。",
  ...
  en: {
    ...
    data: "Guest identifiers, game results, and hidden matchmaking ratings provide the game service. Guest progress merges safely when you register or sign in. Login names remain private; display names, eligible ratings, and leaderboard results are public. Email is optional and used only for account recovery.",
  ...
  ja: {
    ...
    dataTitle: "データとアカウント",
    data: "ゲスト識別子、ゲーム結果、非公開のマッチングレートをサービス提供に使用します。登録またはログイン時にゲスト記録を安全に統合します。ログイン名は非公開で、表示名、公開対象の Elo とランキング結果は公開されます。メールは任意で、アカウント復旧のみに使用します。",
