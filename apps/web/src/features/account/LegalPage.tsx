...
const copy = {
  "zh-CN": {
    ...
    dataTitle: "数据与账号",
    data: "访客标识、游戏结果和隐藏匹配分用于提供游戏。注册或登录时，访客记录会安全合并到账号。登录名保持私密；每日榜会显示访客或注册用户的展示名，Elo 榜显示已注册用户的展示名与评分。邮箱可选，只用于找回密码。",
  ...
  en: {
    ...
    data: "Guest identifiers, game results, and hidden matchmaking ratings provide the game service. Guest progress merges safely when you register or sign in. Login names remain private; the daily board shows guest or registered display names, while the Elo board shows registered display names and ratings. Email is optional and used only for account recovery.",
  ...
  ja: {
    ...
    dataTitle: "データとアカウント",
    data: "ゲスト識別子、ゲーム結果、非公開のマッチングレートをサービス提供に使用します。登録またはログイン時にゲスト記録を安全に統合します。ログイン名は非公開です。デイリーランキングにはゲストまたは登録ユーザーの表示名、Eloランキングには登録ユーザーの表示名とレートが表示されます。メールは任意で、アカウント復旧のみに使用します。",
